    ...
    setInterval(async () => {
        if (!flag) {
            ...
            try {
                ...
                if (findStads.length !== 0) {

                    for (let i = 0; i < findStads.length; i++) {
                        try {
                            const findAddress = await knex(address).where({ id: findStads[i].address_id }).returning('*');

                            const timezone = await findAddress[0].timezone[0];

                            let begin = await DateTime.fromJSDate(findStads[i].begin, "dd.mm.yyyy - hh:ss").toUTC().setZone(timezone);
                            let end = await DateTime.fromJSDate(findStads[i].end, "dd.mm.yyyy - hh:ss").toUTC().setZone(timezone);
                            ...
                            try {
                                const findOpeningHours = await knex(opening_hour)
                                    .where({
                                        address_id: findStads[i].address_id
                                    })
                                    .returning('*')
                                ...
                                if (tzNow < end) {
                                    ...
                                    for (let l = 0; l < weekListTimeFrom.length; l++) {
                                        ...
                                        if (tzNow >= openFrom && tzNow <= openTo) {
                                            if (findAddress[0].active_stad) {
                                                if (!findStads[i].is_active) {
                                                    try {
                                    ...
                                    for (let m = 0; m < disabledDatesFrom.length; m++) {
                                        ...
                                        if (tzNow > disabledFrom && tzNow < disabledTo) {
                                            ...
                                            if (end > disabledFrom && end <= disabledTo) {
                                                if (findStads[i].is_active && findAddress[0].active_stad) {
                                                    try {
                                                        await knex(address).where({
                                                            id: findStads[i].address_id
                                                        })
