...
const getAllSubsubcategorys = async (req, res, next) => {
        ...
        if (id == null) {
            ...
            try {
                ...
                if (findInWorkers.length === 0) {
                    ...
                    try {
                        const allSubsubcategorys = await knex(subsubCategory)
                            .orderBy('name', 'asc')
                            .returning('*');
                        res.json(allSubsubcategorys);
                    } catch (error) {
    ...
    if (permission === process.env.PERMISSION_KEY_ADVERTISER || permission === process.env.PERMISSION_KEY_CUSTOMER || permission === process.env.PERMISSION_KEY_WORKER) {
        if (id == null) {
            ...
            try {
                ...
                if (findInWorkers.length !== 0 || findAdvertiser.length !== 0 || findCustomer.length !== 0) {
                    try {
                        const oneSubsubcategory = await knex(subsubCategory)
                            .where({
                                id: req.params.identifier
                            })
                            .returning('*');
                        res.json(oneSubsubcategory[0]);
                    } catch (error) {
                        next(createError(400, 'Bad request!'));
                    }
                }
...
const getAllActiveSubsubcategorys = async (req, res, next) => {
    const permission = await req.headers.permission;
    if (permission === process.env.PERMISSION_KEY_ADVERTISER || permission === process.env.PERMISSION_KEY_CUSTOMER || permission === process.env.PERMISSION_KEY_WORKER) {
        try {
            const allSubsubcategorys = await knex(subsubCategory)
...
const getAllActiveSubsubcategorysFromCategory = async (req, res, next) => {
    ...
    if (permission === process.env.PERMISSION_KEY_ADVERTISER || permission === process.env.PERMISSION_KEY_CUSTOMER || permission === process.env.PERMISSION_KEY_WORKER) {
        if (id == null) {
            ...
            try {
                ...
                if (findInWorkers.length !== 0 || findAdvertiser.length !== 0 || findCustomer.length !== 0) {
                    try {
                        const allActiveSubsubcategorys = await knex(subsubCategory)
                            .orderBy('name', 'asc')
                            .where({
                                is_active: true,
                            ...
                            .returning('*');
                        res.json(allActiveSubsubcategorys);
                    } catch (error) {
                        next(createError(400, 'Bad request!'));
                    }
                }
...
const getAllActiveSubsubcategorysFromSubcategory = async (req, res, next) => {
                ...
                if (findInWorkers.length !== 0 || findAdvertiser.length !== 0 || findCustomer.length !== 0) {
                    try {
                        const allActiveSubsubcategorys = await knex(subsubCategory)
                            ...
                            .where({
                                is_active: true,
                                subcategory_id: req.params.identifier
                            })
                            .returning('*');
                        res.json(allActiveSubsubcategorys);
                    } catch (error) {
        ...
        if (id == null) {
            ...
            try {
                ...
                if (await findInWorkers.length === 0) {
                    ...
                    try {
                        const createdSubsubcategory = await knex(subsubCategory).insert({
                            ...req.body
                        }).returning('*');

                        if (createdSubsubcategory.length === 0) {
                            next(createError(400, 'Error: Subsubcategory not created!'));
                        } else {
                            res.json(createdSubsubcategory[0]);
                        }
                    } catch (error) {
                        next(error);
                    }
        ...
        if (id == null) {
            ...
            try {
                ...
                if (await findInWorkers.length === 0) {
                    ...
                    try {
                        const findSubsubcategory = await knex(subsubCategory).where({
                            id: req.params.identifier,
                        }).returning('*')

                        if (findSubsubcategory.length === 0) {
                            next(createError(404, 'Error: Subcategory not found.'));
                        } else {
                            try {
                                const updatedSubsubcategory = await knex(subsubCategory).where({
                                    id: req.params.identifier,
                                }).update({
                                    ...req.body,
                                ...
                                if (updatedSubsubcategory.length === 0) {
                                    next(createError(400, 'Error: Subcategory not updated'));
                                } else {
                                    res.json(updatedSubsubcategory[0]);
                                }
                            } catch (error) {
                                next(error);
                            }
...
const getSubSubcategoryStad = async (parent, req, res, next) => {
    const id = await getUserId(req);
    const permission = await req.headers.permission;
    if (permission === process.env.PERMISSION_KEY_ADVERTISER || permission === process.env.PERMISSION_KEY_CUSTOMER || permission === process.env.PERMISSION_KEY_WORKER) {
        if (id == null) {
            ...
            try {
                ...
                if (findInWorkers.length !== 0 || findAdvertiser.length !== 0 || findCustomer.length !== 0) {
                    try {
                        const findStad = await knex(stad_subsubcategorys).where({
                            stad_id: req.params.identifier,
                        })
                            .returning('*');
                        res.json(findStad[0]);
                    } catch (error) {
...
module.exports = {
    ...
    getSubsubcategory,
    getAllActiveSubsubcategorysFromCategory,
    getAllActiveSubsubcategorysFromSubcategory,
    createSubsubcategory,
    updateSubsubcategory,
    getAllActiveSubsubcategorys,
    getSubSubcategoryStad,
};