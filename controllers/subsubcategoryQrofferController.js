...
const getSubsubcategory = async (parent, req, res, next) => {
                ...
                if (findInWorkers.length !== 0 || findAdvertiser.length !== 0 || findCustomer.length !== 0) {
                    try {
                        const findsubsubcategory = await knex(subsubCategory).where({
                            id: parent.subsubcategory_id,
                        })
                            .returning('*');
                        res.json(findsubsubcategory[0]);
                    } catch (error) {
...
const getQroffer = async (parent, req, res, next) => {
    const id = await getUserId(req);
    const permission = await req.headers.permission;
    if (permission === process.env.PERMISSION_KEY_ADVERTISER || permission === process.env.PERMISSION_KEY_CUSTOMER || permission === process.env.PERMISSION_KEY_WORKER) {
        if (id == null) {
...
const getSubSubcategoryQroffer = async (req, res, next) => {
    const id = await getUserId(req);
    const permission = await req.headers.permission;
    if (permission === process.env.PERMISSION_KEY_ADVERTISER || permission === process.env.PERMISSION_KEY_CUSTOMER || permission === process.env.PERMISSION_KEY_WORKER) {
        if (id == null) {
            ...
            try {
                ...
                if (findInWorkers.length !== 0 || findAdvertiser.length !== 0 || findCustomer.length !== 0) {
                    try {
                        const findQroffer = await knex(qroffer_subsubcategorys).where({
                            qroffer_id: req.params.identifier,
                        })
                            .returning('*');
                        await res.json(await findQroffer);
                    } catch (error) {
                        next(createError(400, 'Bad request!'));
                    }
                }
