    ...
    if (permission === process.env.PERMISSION_KEY_ADVERTISER || permission === process.env.PERMISSION_KEY_CUSTOMER || permission === process.env.PERMISSION_KEY_WORKER) {
        if (id == null) {
            ...
            try {
                ...
                if (findInWorkers.length !== 0 || findAdvertiser.length !== 0 || findInCustomer.length !== 0) {
                    try {
                        const allStads = await knex(i_want_it_stad)
                            .returning('*');
                        res.json(allStads);
                    } catch (error) {
...
const getAllIwantItsForOneStad = async (req, res, next) => {
    const id = await getUserId(req);
    const permission = await req.headers.permission;
...
const iWantItStadMe = async (req, res, next) => {
    const id = await getUserId(req);
    const permission = await req.headers.permission;
        ...
        if (id == null) {
            ...
            try {
                ...
                if (findCustomer.length === 0) {
                    ...
                    try {
                        const findIWantIt = await knex(i_want_it_stad)
                            .where({
                                customer_id: id,
                                stad_id: req.params.identifier
                            })
...
const createAndDeleteIWantItStad = async (req, res, next) => {
    const id = await getUserId(req);
    const permission = await req.headers.permission;
        ...
        if (id == null) {
            ...
            try {
                ...
                if (findCustomer.length === 0) {
                    ...
                    try {
                        const findIWantItStad = await knex(i_want_it_stad).where({
                            customer_id: id,
                            stad_id: req.body.stad_id
                        }).returning('id');


                        if (findIWantItStad.length === 0) {
                            try {
                                const createIWantItStad = await knex(i_want_it_stad).insert({
                                    customer_id: id,
                                    stad_id: req.body.stad_id
                                }).returning('*');


                                if (createIWantItStad.length === 0) {
                                    next(createError(400, "Error: Create I want it qroffer"));
                                } else {
                                    res.json(createIWantItStad[0]);
                                }
                            } catch (error) {
                                next(error);
                            }
                            ...
                            try {
                                await knex(i_want_it_stad).where({
                                    customer_id: id,
                                    stad_id: req.body.stad_id
                                }).del();
                            } catch (error) {
...
module.exports = {
    getAllIwantIts,
    iWantItStadMe,
    createAndDeleteIWantItStad,
    getAllIwantItsForOneStad,
}